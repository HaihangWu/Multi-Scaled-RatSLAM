    ...
    // find the closest experience in pose cell space
      ...
      if (current_exp != (unsigned)matched_exp_id)
      {
        ...
        else
        {
          ...
          if (action == NO_ACTION)  //????? meaningless
          {
            action = SET_NODE;
          }
        }
...
void PosecellNetwork::on_view_template(unsigned int vt, double vt_rad)
{
  ...
  else
  {
    ...
    if (vt < (visual_templates.size() - 10))
    {
      ...
      if (energy > 0)
      {
		...
		double pc_th_corrected = pcvt->pc_th + vt_rad / (2.0*M_PI) * PC_DIM_TH; // if the image has rotation angle from the original image, then rotate robot angle
		if (pc_th_corrected < 0) 
			pc_th_corrected = PC_DIM_TH + pc_th_corrected;
		if (pc_th_corrected >= PC_DIM_TH)
			pc_th_corrected = pc_th_corrected - PC_DIM_TH;
  ...
  for (unsigned int i=0; i < visual_templates.size(); i++) //std::vector<PosecellVisualTemplate>
  {
...
double MultiplePosecellNetwork::get_min_delta_mul(double d1, double d2, double max)
{
  double absval = abs(d1 - d2);
  return min(absval, max - absval);
}
...
MultiplePosecellNetwork::MultiplePosecellAction MultiplePosecellNetwork::get_action()
{
  ...
  if (experiences_multiple.size() == 0)
  {
   ...
   if (delta_pc_mul > EXP_DELTA_PC_THRESHOLD_MUL)
    {
      // go through all in the experiences_multiple the  and find the one with the closest delta_pc
      int matched_exp_id = -1;
      unsigned int i;
      int min_delta_id = -1;
      double min_delta = DBL_MAX;
    ...
    // find the closest experience in pose cell space
     // i=10000000;
      ...
      if (current_exp_mul != (unsigned)matched_exp_id)
      {
        if (matched_exp_id == -1)
        {
          create_experience();
        ...
        {
          current_exp_mul = matched_exp_id;
          if (action == NO_ACTION)  //????? meaningless
          {
            action = SET_NODE;
          }
