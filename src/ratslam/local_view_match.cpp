...
{
  get_setting_from_ptree(VT_MIN_PATCH_NORMALISATION_STD, settings, "vt_min_patch_normalisation_std", (double)0);// the value std in a patch 
  get_setting_from_ptree(VT_PATCH_NORMALISATION, settings, "vt_patch_normalise", 0); //patch_size or the number of blocks in a patch
  ...
  get_setting_from_ptree(TEMPLATE_X_SIZE, settings, "template_x_size", 1);// Number of "pixels" in template 
  get_setting_from_ptree(TEMPLATE_Y_SIZE, settings, "template_y_size", 1);// Number of "pixels" in template 
  get_setting_from_ptree(IMAGE_VT_X_RANGE_MIN, settings, "image_crop_x_min", 0); // this parameter may need to be adjusted
  get_setting_from_ptree(IMAGE_VT_X_RANGE_MAX, settings, "image_crop_x_max", -1);// this parameter may need to be adjusted
  get_setting_from_ptree(IMAGE_VT_Y_RANGE_MIN, settings, "image_crop_y_min", 0);// this parameter may need to be adjusted
  get_setting_from_ptree(IMAGE_VT_Y_RANGE_MAX, settings, "image_crop_y_max", -1);// this parameter may need to be adjusted
...
void LocalViewMatch::on_image(const unsigned char *view_rgb, bool greyscale, unsigned int image_width, unsigned int image_height)
{
  ...
  convert_view_to_view_template(greyscale); // step 1-4 in 3.2
  prev_vt = get_current_vt();
  unsigned int vt_match_id;
  compare(vt_error, vt_match_id); //the minimum comparison error between current view and the templates
...
void LocalViewMatch::convert_view_to_view_template(bool grayscale)
{
  ...
  if (VT_NORMALISATION > 0) // address the global change in illumination
  {
    double avg_value = 0;
